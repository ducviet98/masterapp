...
import {
  ...
  makeSelectIsLoading,
  makeSelectTotal,
} from 'src/containers/MfiToken/store/selectors';
...
const MfiContainer = () => {
  ...
  const selectItemTable = (value: number) => {
    if (selectedItems.includes(value))
      return setSelectedItems(selectedItems.filter((item: any) => item !== value));
  ...
  const renderBodyTable = () =>
    mfiTokens?.map((row: any) => ({
      id: row.id,
      certificate_id: (
        <Link component={RouterLink} to={`${path.mfiToken}/${row.id}`} variant="subtitle2" noWrap>
          {row.certificate_id}
        </Link>
      ),
      name: row.name,
    ...
    <Page title="MFI Token List">
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs
          ...
          links={[{ name: 'Dashboard', href: '/' }, { name: 'MFI Token' }]}
          action={
            <Button
              variant="contained"
